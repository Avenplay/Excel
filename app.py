...
def inicializar_base_datos():
    ...
    try: cursor.execute("ALTER TABLE despensa ADD COLUMN fecha_compra TEXT")
    except: pass
    cursor.execute("""
...
def ejecutar_automatizaciones_mensuales():
    ...
    for nombre, monto in cursor.fetchall():
        ...
        if cursor.fetchone()[0] == 0:
            cursor.execute("INSERT INTO movimientos_caja (fecha, concepto, monto, tipo_ingreso_gasto, metodo_pago) VALUES (?, ?, ?, 'Gasto Habitual', 'Tarjeta/PayPal')", (datetime.now().strftime("%Y-%m-%d"), concepto_cargo, monto))
    ...
    for pid, articulo, total, m_totales, m_pagados in cursor.fetchall():
        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute("INSERT INTO movimientos_caja (fecha, concepto, monto, tipo_ingreso_gasto, metodo_pago) VALUES (?, ?, ?, 'Gasto Habitual', 'Tarjeta/PayPal')", (datetime.now().strftime("%Y-%m-%d"), concepto_cuota, total / m_totales))
...
def obtener_totales_sistema():
    ...
    return (ingresos_fijos + extras_banco - gastos_tarjeta), (extras_efectivo - gastos_efectivo), extras_banco, excepcionales, coste_comida, mermas_comida, total_recurrentes, total_cuotas_plazos
...
def registrar_movimiento(concepto, monto, tipo, metodo, subcuenta='N/A'):
    conexion = sqlite3.connect(DB_PATH, timeout=10)
    cursor = conexion.cursor()
    cursor.execute("INSERT INTO movimientos_caja (fecha, concepto, monto, tipo_ingreso_gasto, metodo_pago, subcuenta_extra) VALUES (?, ?, ?, ?, ?, ?)", (datetime.now().strftime("%Y-%m-%d"), concepto, monto, tipo, metodo, subcuenta))
    conexion.commit(); conexion.close()
...
if opcion_menu == "💵 Control de Caja":
    ...
    with col_l:
        ...
        with st.form("form_gasto"):
            ...
            if st.form_submit_button("Guardar Gasto") and concepto_g and monto_g > 0:
                registrar_movimiento(concepto_g, monto_g, tipo_g, metodo_g); st.rerun()
    with col_r:
        ...
        with st.form("form_ingreso"):
            ...
            if st.form_submit_button("Guardar Entrada") and concepto_i and monto_i > 0:
                registrar_movimiento(concepto_i, monto_i, tipo_i, metodo_i, subcuenta_i); st.rerun()
    st.markdown("---")
    ...
    if not df_movimientos.empty:
        ...
        running_banco, running_hucha, saldos_registro = 0.0, 0.0, {}
        for idx, row in df_chrono.iterrows():
            ...
            if tipo == 'Ingreso Fijo': running_banco += monto
        ...
        for index, fila in df_movimientos.iterrows():
            ...
            with c_detalles: st.write(f"📅 **{m_fecha}** | `{m_tipo}` | **{m_concepto}** -> **{m_monto:,.2f} €** ({m_pago}) | {txt_balance}")
            with c_eliminar:
                if st.button("🗑️ Borrar", key=f"del_mov_{m_id}"):
                    conexion = sqlite3.connect(DB_PATH, timeout=10); conexion.execute("DELETE FROM movimientos_caja WHERE id = ?", (m_id,)); conexion.commit(); conexion.close(); st.rerun()
        ...
        for index, fila in df_stock.iterrows():
            ...
            with c_btn2:
                if st.button(f"🗑️ Tirar / Merma", key=f"tir_{id_prod}"):
                    ...
                    conexion.commit(); conexion.close(); st.rerun()
            st.markdown("<hr style='margin:0.2rem 0px;'/>", unsafe_allow_html=True)
    else: st.info("No hay alimentos en la despensa.")
        ...
        for index, row in utensilios.iterrows():
            ...
            with c2:
                if st.button("🛒 Gastado (A Lista)", key=f"ut_lista_{row['id']}"):
                    ...
                    conexion = sqlite3.connect(DB_PATH, timeout=10); conexion.execute("DELETE FROM utensilios WHERE id = ?", (row['id'],)); conexion.commit(); conexion.close(); st.rerun()
            with c3:
                if st.button("🗑️ Borrar", key=f"ut_del_{row['id']}"):
                    conexion = sqlite3.connect(DB_PATH, timeout=10); conexion.execute("DELETE FROM utensilios WHERE id = ?", (row['id'],)); conexion.commit(); conexion.close(); st.rerun()
            st.markdown("<hr style='margin:0.2rem 0px;'/>", unsafe_allow_html=True)
    else: st.info("No tienes utensilios registrados.")
    ...
    with st.form("form_lista_manual"):
        ...
        with col3: 
            ...
            if st.form_submit_button("Añadir a la Lista") and prod_manual:
                añadir_a_lista_compra(prod_manual.capitalize(), super_manual); st.rerun()
    st.markdown("---")
    conexion = sqlite3.connect(DB_PATH, timeout=10)
    ...
    if not lista_df.empty:
            ...
            for _, row in productos_super.iterrows():
                ...
                with c2: 
                    if st.button("❌", key=f"del_list_{row['id']}"):
                        conexion = sqlite3.connect(DB_PATH); conexion.execute("DELETE FROM lista_compra WHERE id=?", (row['id'],)); conexion.commit(); conexion.close(); st.rerun()
        ...
        def limpiar_bd_lista():
            ...
            conn_limpia.commit(); conn_limpia.close()

        st.download_button(label="📥 Descargar TXT para el Móvil y Limpiar Lista", data=texto_export, file_name="Lista_Compra.txt", mime="text/plain", on_click=limpiar_bd_lista)
    else: st.success("¡Tu lista de la compra está vacía!")
    ...
    with col_izq:
        with st.form("form_add_recurrente"):
            ...
            if st.form_submit_button("Registrar") and nombre_fijo and monto_fijo > 0:
                try:
                    conexion = sqlite3.connect(DB_PATH, timeout=10); conexion.execute("INSERT INTO gastos_recurrentes (nombre_gasto, monto) VALUES (?, ?)", (nombre_fijo, monto_fijo)); conexion.commit(); conexion.close(); st.rerun()
    ...
    with col_l:
        with st.form("form_plazos"):
            ...
            if st.form_submit_button("Auditar y Registrar"):
                ...
                if cuota < 25.0: st.error(f"🚨 Cuota de {cuota:.2f} €/mes. Regla rota. ¡A tocateja!")
                else:
                    try:
                        conexion = sqlite3.connect(DB_PATH, timeout=10); conexion.execute("INSERT INTO compras_plazos (articulo, monto_total, meses_totales, meses_pagados) VALUES (?, ?, ?, 0)", (art_nombre, art_total, art_meses)); conexion.commit(); conexion.close(); st.rerun()
    ...
    with st.form("form_proyecto"):
        ...
        if st.form_submit_button("Lanzar Proyecto") and proj_name:
            try:
                conexion = sqlite3.connect(DB_PATH, timeout=10); conexion.execute("INSERT INTO proyectos_futuros (nombre_proyecto, objetivo_total, meses_restantes) VALUES (?, ?, ?)", (proj_name, proj_target, proj_months)); conexion.commit(); conexion.close(); st.rerun()
            except: st.error("Ya existe.")
    conexion = sqlite3.connect(DB_PATH, timeout=10)
    ...
    for index, fila in df_proj.iterrows():
        ...
        with col_add2:
            ...
            if st.button("Confirmar", key=f"btn_h_{fila['id']}") and abono > 0:
                ...
                conexion.execute("INSERT INTO movimientos_caja (fecha, concepto, monto, tipo_ingreso_gasto, metodo_pago) VALUES (?, ?, ?, 'Gasto Habitual', 'Tarjeta/PayPal')", (datetime.now().strftime("%Y-%m-%d"), f"Abono hucha: {fila['nombre_proyecto']}", abono))
                conexion.commit(); conexion.close(); st.rerun()
        with col_add3:
            st.write("")
